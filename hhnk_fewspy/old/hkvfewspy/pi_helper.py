...
class SetPiTimeSeries(object):
    ...
    def _pi_xml(self):
        ...
        if "timeZone" in obj:
            SubElement(root, "timeZone").text = obj["timeZone"]
        if "geoDatum" in obj:
            SubElement(root, "geoDatum").text = obj["geoDatum"]

        # COMMENT child
        if "c" in locals():
            ch1 = SubElement(root, "series")
            if c:
                SubElement(ch1, "comment").text = c

        # HEADER child
        if "h" in locals():
            sbch1 = SubElement(ch1, "header")
            if "type" in h:
                SubElement(sbch1, "type").text = h["type"]
            if "moduleInstanceId" in h:
                SubElement(sbch1, "moduleInstanceId").text = h["moduleInstanceId"]
            if "locationId" in h:
                SubElement(sbch1, "locationId").text = h["locationId"]
            if "parameterId" in h:
                SubElement(sbch1, "parameterId").text = h["parameterId"]
            if "qualifierId" in h:
                # FIXME updated add option for qualifier list
                if type(h["qualifierId"]) == list:
                    for quali in h["qualifierId"]:
                        SubElement(sbch1, "qualifierId").text = quali
                else:
                    SubElement(sbch1, "qualifierId").text = h["qualifierId"]

            if "ensembleId" in h:
                SubElement(sbch1, "ensembleId").text = h["ensembleId"]
            if "timeStep" in h:
            ...
            if "missVal" in h:
                SubElement(sbch1, "missVal").text = str(h["missVal"])
            if "longName" in h:
                SubElement(sbch1, "longName").text = str(h["longName"])
            if "stationName" in h:
                SubElement(sbch1, "stationName").text = str(h["stationName"])
            if "lat" in h:
                SubElement(sbch1, "lat").text = str(h["lat"])
            if "lon" in h:
                SubElement(sbch1, "lon").text = str(h["lon"])
            if "x" in h:
                SubElement(sbch1, "x").text = str(h["x"])
            if "y" in h:
                SubElement(sbch1, "y").text = str(h["y"])
            if "z" in h:
                SubElement(sbch1, "z").text = str(h["z"])
            if "units" in h:
                SubElement(sbch1, "units").text = str(h["units"])
            if "sourceOrganisation" in h:
                SubElement(sbch1, "sourceOrganisation").text = str(h["sourceOrganisation"])
            if "sourceSystem" in h:
                SubElement(sbch1, "sourceSystem").text = str(h["sourceSystem"])
            if "fileDescription" in h:
                SubElement(sbch1, "fileDescription").text = str(h["fileDescription"])
            if "creationDate" in h:
                SubElement(sbch1, "creationDate").text = str(h["creationDate"])
            if "creationTime" in h:
                SubElement(sbch1, "creationTime").text = str(h["creationTime"])
            if "region" in h:
                SubElement(sbch1, "region").text = str(h["region"])
            if "thresholds" in h:
        ...
        if "p" in locals():
            ...
            if "description" in p:
                SubElement(sbch2, "description").text = p["description"]
            if "string" in p:
        ...
        if "e" in locals():
            for row in e.iterrows():
                ...
                if "value" in row[1].index:
                    sbch3.set("value", str(row[1].value))
                if "minValue" in row[1].index:
                    sbch3.set("minValue", str(row[1].minValue))
                if "maxValue" in row[1].index:
                    sbch3.set("maxValue", str(row[1].maxValue))
                if "flag" in row[1].index:
                    sbch3.set("flag", str(row[1].flag))
                if "flagSource" in row[1].index:
                    sbch3.set("flagSource", str(row[1].flagSource))
                if "comment" in row[1].index:
                    sbch3.set("comment", str(row[1].comment))
                if "user" in row[1].index:
                    sbch3.set("user", str(row[1].user))
    ...
    class _Root(object):
        def _update_root(self, key, value):
            try:
                self._outer.view.update({key: value})
    ...
    class _Comment(object):
        def _update_comment(self, key, value):
            try:
                self._outer.view["timeSeries"][0].update({key: value})
            except KeyError as e:
    ...
    class _Properties(object):
        def _update_properties(self, key, value):
            try:
                self._outer.view["timeSeries"][0]["properties"].update({key: value})
            except KeyError as e:
    ...
    class _Header(object):
        def _update_header(self, key, value):
            try:
                self._outer.view["timeSeries"][0]["header"].update({key: value})
            except KeyError as e:
                if e.args[0] == "header":
    ...
    class _Events(object):
        ...
        def events(self, value):
            ...
            if (
                not value.index.inferred_type == "datetime64"
            ):  # FIXME updated from original .is_all_dates gave warning.
            ...
            if len(inv_names):
                inv_message = " The following column names are ignored: '{}'. \n Only the following column names are accepted: '{}''".format(
                    "', '".join(inv_names), "', '".join(accepted_names)
                )
...
class Utils(object):
    ...
    def addFilter(self, child):
        setattr(
            self.Filters,
            child["id"].replace(".", "_"),
            {"id": child["id"], "name": child.name.cdata, "description": child.description.cdata},
        )
    ...
    def event_client_datetime(event, tz_server, tz_client="Europe/Amsterdam"):
        ...
        event_server_time = list(map(int, event["time"].replace(".", ":").split(":")))  # -> [HH, mm, ss, ms]
...
def read_timeseries_response(getTimeSeries_response, tz_client, header="longform"):
    ...
    if header in ["multiindex", "longform"]:
        ...
        metacolumns = ["moduleInstanceId", "qualifierId", "parameterId", "units", "locationId", "stationName"]

        datacolumns = ["datetime", "flag", "value", "user"]
        ...
        for series in getTimeSeries_json.TimeSeries.series:
            ...
            if hasattr(series, "event"):
                ...
                # GET data values
                times = [
                    (*map(int, re.split("-", event["date"])), *map(int, re.split(":|\.", event["time"])))
