...
class Series(object):
    ...
    def __init__(self, tree, start=None, end=None, step=None, ma=None, missval=None):
        """
        ...
        if ma is None:
            size = int((self.end - self.start).total_seconds() / self.step.total_seconds()) + 1
    ...
    def _get_tree_value(self, tag):
        ...
        if tag in (TAG_START, TAG_END):
            ...
            td_kwargs = {
                "{}s".format(element.attrib["unit"]): int(element.attrib["multiplier"]),
            }
    ...
    def read(self):
        ...
        for parse_event, elem in iterator:
            ...
            # New series. Copy to series, remove unwanted children.
                ...
                tree.append(series)
                map(series.remove, series.iter())  # FIXME was series.getchildren()[:]
            # Timezone should be in the copy of the tree
                ...
                map(tree.remove, tree.iter())  # FIXME tree.getchildren()[:]
...
class SeriesWriter(object):
    ...
    def _register_namespace(self, series):
        ...
        namespace = re.search("\{([^{}]*)\}([^{}]*)", series.tree.tag).group(1)
    ...
    def write(self, series_iterable):
        ...
        for series in series_iterable:
            ...
            if not self.initialized:
                ...
                self.xml_output_file.write('<?xml version="1.0" encoding="UTF-8"?>\n')
